  ...
  const handleSaveBranding = async () => {
    try {
      if (USE_LOCAL_API) {
        ...
        // If we have an existing branding record, update it. Otherwise insert it.
        const { data: existing } = await supabase.from('branding' as any).select('id').maybeSingle();
        
        const payload = { ...formData };
        if (existing) {
          const { error } = await supabase.from('branding' as any).update(payload).eq('id', existing.id);
          if (error) throw error;
        } else {
          const { error } = await supabase.from('branding' as any).insert([payload]);
          if (error) throw error;
        }
  ...
  const handleUploadMedia = async () => {
    ...
    try {
      if (USE_LOCAL_API) {
        ...
        const { error } = await supabase.from('media' as any).insert([mediaFormData]);
        if (error) throw error;
